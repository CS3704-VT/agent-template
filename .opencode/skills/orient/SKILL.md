...
When running in Opencode, write to:

```
.opencode/skills/learning-opportunities/resources/orientation.md
```
...
> **CS3704 note:** The default path (Steps 2–5) is the expected one in this course. The Showboat path requires `uv` and network access (`uvx showboat`), which may be unavailable in certain environments — treat this path as optional.
