...
- Run the full MCII exercise. Ask the student why they are taking this course, what they hope to understand and become able to do, what would make this course worthwhile, how will they recognize when they learn something, what resources will help their learning, and what might interfere with their learning. 

<!--- Questions derived from Amy Allen and David Hicks, School of Education at Virginia Tech, AI Teaching Symposium 2026-->

### 2. Per-Assignment goal (each assignment)
- Store at: ./learning-goal.md (project root)
- Run a shorter version — focus on what they want to get out of this specific assignment (use assignment instructions for context in the per-assignment `<name>.md` file, e.g. `PM1.2.md`). Ask if they also want to build on their semester goal or set something new.

### 3. Team project goal (per project)
- Store at: ./team-goal.md (project root)
- Run with the project team present. Ask the team to agree on a shared goal. The if-then plans should focus on team dynamics (e.g., "If we disagree on the approach, then we will...").

**Always ask the user which goal scope they want to work on**.
...
Ask the learner to name a specific skill or area they want to grow in. Encourage specific goals that connect to their real lives and meaningful outcomes. Instead of "get better at coding," goals such as "develop my frontend skills so that I can prepare for a future role," or "learn enough python to pursue a project." 
...
**Relevant.** Does this connect to something they actually care about, such as their work, a project, their role-based identity? Goals that are "should" goals ("I should learn this because everyone says it looks good on a resume") tend to lose to goals that are "want" goals ("I want to actually understand how the code I write turns into something that runs"). Ask: "How does this connect to your larger priorities?"
...
"Here's what one might look like: 'If I open my laptop to study and feel the pull to check Instagram, TikTok, Youtube, email, etc. first, then I will close the application and open my project file before doing anything else.' Now try one with the obstacle you described."
